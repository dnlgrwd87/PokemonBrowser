<template>
    <div v-if="evo.stage > 1">
        ...
        <span
            v-if="!mobile"
            class="evo-info has-text-weight-light has-text-centered"
            v-html="getEvoText(evo)"
        ></span>
        <span
            v-if="mobile"
            class="evo-info has-text-weight-light has-text-centered"
            v-html="getEvoText(evo, true)"
        ></span>
    </div>
</template>
...
export default {
    ...
    computed: {
        ...
        showNormalArrows() {
            return this.evo.name === 'beautifly' || this.evo.name === 'dustox';
        },
    },
};
</script>
