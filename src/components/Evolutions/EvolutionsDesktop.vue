<template>
    <div>
        <div
            ...
            :class="{
                'is-mobile': allStages.length === 2,
                'is-hidden-mobile': allStages.length > 2,
            }"
        >
            <div
                v-for="(stage, index) in allStages"
                :key="index"
                class="column has-text-centered evo-column"
            >
                <div
                    v-for="(evo, evoIndex) in stage"
                    :key="evo.name"
                    class="current-evo columns is-centered is-vcentered"
                >
                    <pokemon-grid-item
                    ...
                    <evolution-arrows
                        ...
                        :stage="stage"
                        :evoIndex="evoIndex"
                        class="desktop-arrows is-hidden-mobile"
                    />
                    <evolution-arrows
                        v-if="allStages.length === 2"
                        :chainLength="allStages.length"
                        :mobile="true"
                        :evo="evo"
                        :stage="stage"
                        :evoIndex="evoIndex"
                        class="two-stage-mobile is-hidden-tablet"
                    />
                </div>
            </div>
        </div>
