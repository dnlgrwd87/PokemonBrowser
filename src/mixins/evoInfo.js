...
// TODO: Clean this up. Possibly store this data in the database
export default {
    mixins: [convertName],
    methods: {
        getEvoText(evo, mobile) {
            ...
            if (evo.name === 'shedinja' && !mobile) {
                text =
                    'Level ' +
                    evo.min_level +
                    ',<br />open spot in party,<br />Pokéball in bag';
                return text;
            }
            ...
            if (evo.name === 'shedinja' && mobile) {
                text =
                    'Lvl ' +
                    evo.min_level +
                    ',<br />spot in party,<br />Pokéball in bag';
                return text;
            }
            ...
            if (evo.trigger === 'level-up') {
                ...
                if (evo.known_move && !mobile) {
                    text =
                        'Level up<br />knowing ' +
                        this.convertName(evo.known_move);
                    return text;
                }

                if (evo.known_move && mobile) {
                    text =
                        'Lvl up<br />knowing<br />' +
                        this.convertName(evo.known_move);
                    return text;
                }
            ...
            if (evo.trigger === 'trade') {
                if (evo.held_item && !mobile) {
                    text =
                        'Trade,<br />holding ' +
                        this.convertName(evo.held_item);
                    return text;
                }
                if (evo.held_item && mobile) {
